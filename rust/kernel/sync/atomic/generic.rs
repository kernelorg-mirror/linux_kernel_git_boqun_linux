...
use super::ops::{AtomicArithmeticOps, AtomicBasicOps, AtomicExchangeOps, AtomicImpl, AtomicRepr};
...
///
/// ```
/// use kernel::sync::atomic::{generic::AtomicType, Atomic, Relaxed};
///
...
impl<T: AtomicType> Atomic<T> {
    ...
    pub const fn as_ptr(&self) -> *mut T {
        // GUARANTEE: Per the safety requirement of `AtomicType`, `T` and `T::Repr` has the same
        // size and alignment, since `self.0` is transparent to `T::Repr`, then `self.0` has the
        // same alignment of `T`, and `self.0` is a valid T per the type invariants.
        self.0.as_ptr().cast()
    }
    ...
    T::Repr: AtomicBasicOps,
{
    ...
    pub fn store<Ordering: ordering::ReleaseOrRelaxed>(&self, v: T, _: Ordering) {
        ...
        // INVARIANTS: `v` is a valid `T`, and is stored to `self.0` by `atomic_set*()`.
        match Ordering::TYPE {
    ...
    T::Repr: AtomicExchangeOps,
{
    ...
    pub fn xchg<Ordering: ordering::Ordering>(&self, v: T, _: Ordering) -> T {
        ...
        // INVARIANT: `*self` remains valid after `atomic_xchg*()` because `v` is transmutable to
        // `T`.
        let ret = {
            match Ordering::TYPE {
    ...
    fn try_cmpxchg<Ordering: ordering::Ordering>(&self, old: &mut T, new: T, _: Ordering) -> bool {
        ...
        // INVARIANTS: `*self` remains valid after `atomic_try_cmpxchg*()` because `new` is
        // transmutable to `T`.
        let ret = {
            match Ordering::TYPE {
