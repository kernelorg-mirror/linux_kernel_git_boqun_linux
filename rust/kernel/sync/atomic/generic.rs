...
impl<T: AtomicType> Atomic<T> {
    ...
    pub const fn as_ptr(&self) -> *mut T {
        // GUARANTEE: `self.0` has the same alignment of `T`, and `self.0` is always a valid T.
        self.0.as_ptr().cast()
    }
    ...
    pub fn get_mut(&mut self) -> &mut T {
        // SAFETY: `self.0` has the same alignment of `T`, and `self.0` is always a valid T.
    ...
    T::Repr: AtomicBasicOps,
{
    ...
    pub fn load<Ordering: ordering::AcquireOrRelaxed>(&self, _: Ordering) -> T {
        ...
        // SAFETY: `v` comes from reading `*self`, which is a valid `T` per type invariants.
        unsafe { from_repr(v) }
    }
