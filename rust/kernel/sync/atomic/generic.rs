...
///   another. Usually this means [`Self`] needs to be [`Send`], but there is a special case:
///   because `*mut T` is not [`Send`] and is yet the basic type needs to support atomic
///   operations, so this safety requirement is added to [`AllowAtomic`] trait. This safety
///   requirement is automatically satisfied if the type is a [`Send`].
///
...
// round-trip transmutable to itself. And all `AtomicImpl` types are `Send`.
...
/// # Safety
///
/// Implementers need to make sure it's safe to transmute back from an arithmetic, that is for a
/// `T: AllowAtomicArithmetic`, [`transmute()`] from `T::Repr` to `T` should always be safe.
///
/// [`transmute()`]: core::mem::transmute
...
// SAFETY: `T` is always safe to `trasnmute()` to itself.
unsafe impl<T: AtomicImpl + AtomicHasArithmeticOps> AllowAtomicArithmetic for T {
    type Delta = Self;
    ...
    T::Repr: AtomicHasArithmeticOps,
{
    ...
    pub fn fetch_add<Ordering: Any>(&self, v: T::Delta, _: Ordering) -> T {
        ...
        // SAFETY: The safety requirement of `AllowAtomicArithmetic` makes it safe to call
        // `from_repr()`.
        unsafe { T::from_repr(ret) }
    }
}
