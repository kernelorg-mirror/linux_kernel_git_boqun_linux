...
static inline bool usage_match(struct lock_list *entry, void *bit)
{
	...
	return (usage & mask) || /* write usage already counts */
	       (!entry->only_xr && (usage & read_mask));
}
 ...
 * a match.
 *
 * Since we already find a match, if it's a read usage, that means we don't
 * have a write usage (and ->only_xr is false, although not need to check that,
