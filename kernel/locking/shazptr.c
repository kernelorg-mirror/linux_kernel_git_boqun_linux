...
struct shazptr_scan {
	...
	struct mutex lock;
	struct list_head queued;		/* __guard_by(lock) */
	bool scheduled;				/* __guard_by(lock) */
...
static int __noreturn shazptr_scan_kthread(void *arg)
{
	struct shazptr_scan *scan = (struct shazptr_scan *)arg;
	for (;;) {
		swait_event_idle_exclusive(scan->wq,
					   READ_ONCE(scan->scheduled));
...
{
	struct shazptr_scan *scan = NULL;
	struct task_struct *t;

	scan = kzalloc(sizeof(*scan) + nr_cpu_ids * sizeof(scan->snaps[0]),
		       GFP_KERNEL);
	...
	t = kthread_run(shazptr_scan_kthread, shazptr_scan, "shazptr_scan");
	if (!IS_ERR(t)) {
		scan->thread = t;
		/* Pair with the smp_load_acquire() in synchronize_shazptr() */
		smp_store_release(&shazptr_scan, scan);
		return 0;
	} else {
		kfree(scan);
	}
...
static void synchronize_shazptr_normal(void *ptr, struct shazptr_scan *scan)
{
	int cpu;
	...
	if (unlikely(skip_synchronize_self_scan)) {
		...
		for_each_possible_cpu(cpu) {
			...
			if (val == ptr || val == SHAZPTR_WILDCARD)
				blocking_grp_mask |= 1UL << (cpu / scan->cpu_grp_size);
		}
	}
...
{
	/* Pair with the smp_store_release() in shazptr_scan_init() */
	struct shazptr_scan* scan = smp_load_acquire(&shazptr_scan);
	if (!scan)
		synchronize_shazptr_busywait(ptr);
	else
		synchronize_shazptr_normal(ptr, scan);
}
